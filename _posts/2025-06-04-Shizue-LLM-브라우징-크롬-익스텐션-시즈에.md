...
&nbsp;&nbsp;부전공이었던 산업디자인과 수업에서 교수님이 하셨던 말이 기억에 남았다. "라이터나 우산처럼 반세기 이상 바뀌지 않은 디자인을 마스터피스라 부른다." 내 생각에 '책'도 그 범주에 속한다. 원하는 페이지를 펼쳐 글을 읽고, 기록한다. 하지만 손을 쓰지 않고 효율적으로 비를 피하는 방법이 등장한다면 우산의 형태는 언제든 바뀔 수 있다. 책도 마찬가지다. 하위 호환성이라는 제약이 있지만, 언젠가 변화할 여지가 있다. Cursor 같은 제품은 내게 그런 변화의 가능성으로 보였다. 점진적인 기능 개선으로 사용자를 끌어들였고, 비동기 에이전트가 고도화되면 페이지 중심이 아닌 작업 흐름 중심의 새로운 UX도 가능할 것이다. 브라우저도 코드 에디터처럼 '책'의 한 형태로 변화할 수 있는 플랫폼이라 생각한다.
