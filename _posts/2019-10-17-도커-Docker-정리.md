...
comments: true
title: "도커 정리"
image_alt: "도커 정리 — 글 공유 카드"
image: "/assets/images/social/201910170.png"
description: "Docker와 컨테이너를 공부하며 정리한 노트입니다. 가상화와 컨테이너의 차이, Windows 실행 환경, 이미지 레이어, 스토리지 드라이버와 볼륨을 다룹니다."
key: 201910170
...
![호스트 위에서 운영체제 자원을 분리해 실행하는 컨테이너 구조](https://cdn-media-1.freecodecamp.org/images/y4w7Yvlj0aGyKUDYWnFJLf8jfyUkyKgVfGDN)
...
- ![Hyper-V의 하이퍼바이저와 부모·자식 파티션 구조](https://msdnshared.blob.core.windows.net/media/TNBlogsFS/BlogFileStorage/blogs_technet/koalra/WindowsLiveWriter/HypervisorWindowsServervirtualization_E32B/clip_image008_thumb.jpg)
...
![Moby Linux VM을 이용하는 Docker for Windows 구조](https://docs.microsoft.com/ko-kr/virtualization/windowscontainers/deploy-containers/media/mobyvm.png)

LCOW를 사용하면 아래와 같은 구조를 띔.

![Windows에서 Linux 컨테이너를 실행하는 LCOW 구조](https://docs.microsoft.com/ko-kr/virtualization/windowscontainers/deploy-containers/media/lcow-approach.png)
...
![읽기 전용 이미지 레이어 위에 쓰기 가능한 컨테이너 레이어를 쌓는 구조](https://docs.docker.com/storage/storagedriver/images/container-layers.jpg)

**Container**

![여러 컨테이너가 읽기 전용 이미지 레이어를 공유하는 구조](https://docs.docker.com/storage/storagedriver/images/sharing-layers.jpg)
...
![Docker 스토리지 드라이버를 비교한 표](https://wiki.krafton.com/download/attachments/132278196/%EC%BA%A1%EC%B2%98.PNG?version=1&modificationDate=1570694559967&api=v2)
...
![OverlayFS의 lowerdir·upperdir·merged 디렉터리 구성](https://docs.docker.com/storage/storagedriver/images/overlay_constructs.jpg)
...
![Docker의 volume·bind mount·tmpfs mount 저장 위치 비교](https://docs.docker.com/storage/images/types-of-mounts.png)
